...
if(isset($_POST['text'])){
    $text = $_POST['text'];
    // $newDate = date("Y-m-d", strtotime($text));

        // $sql = "SELECT * FROM report WHERE date LIKE '$text'";
        $sql = "SELECT * FROM report AS r JOIN employee AS e ON R.employee_id = e. employee_id WHERE str_to_date(date, '%Y-%m-%d') = '$text'";
        ...
        if($rows > 0){
           ...
           <?php }
            ...
            <div id="row">No report on that day</div
       <?php }
}
?>